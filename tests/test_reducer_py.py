...
def test_negative_bounds(cl_env, program, coord_dtype):
    ...
    values = np.random.normal(-10, 1, size=(24, 4)).astype(coord_dtype).clip(max=-1.0)
    ...
    np.testing.assert_equal(out_buf[:, :3], expected[:, :3])
...
def test_bounds(cl_env, program, coord_dtype, size, ngroups, group_size):
    ...
    values = np.random.normal(size=(size, 4)).astype(coord_dtype)
...
def test_auto_program(cl_env, size, ngroups, group_size, coord_dtype):
    ...
    values = np.random.normal(scale=1e8, size=(size, 4)).astype(coord_dtype)
