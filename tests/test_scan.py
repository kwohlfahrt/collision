...
def test_scan(cl_env, scan_kernels):
    ...
    values = np.array(
        [17, 6, 24, 28, 18, 22, 2, 1, 25, 17, 7, 17, 3, 19, 8, 23], dtype="uint32"
    )
    ...
    block_sums_buf = cl.Buffer(
        ctx, cl.mem_flags.READ_WRITE, nblocks * values.dtype.itemsize
    )
...
def test_block_scan(cl_env, scan_kernels):
    ...
    values = np.array(
        [0, 17, 23, 47, 75, 93, 115, 117, 0, 25, 42, 49, 66, 69, 88, 96], dtype="uint32"
    )
