...
def test_collision(cl_env, coord_dtype, collision_programs):
    ...
    e = collider.get_collisions(
        cq, coords_buf, radii_buf, n_collisions_buf, collisions_buf, len(expected)
    )
...
@pytest.mark.parametrize(
    "size,ngroups,group_size", [(120, 5, 8), (256, 4, 32), (317, 4, 16), (341, 4, 64)]
)
def test_random_collision(
    cl_env, coord_dtype, collision_programs, size, ngroups, group_size
):
    ctx, cq = cl_env
    collider = Collider(
        ctx, size, ngroups, group_size, coord_dtype, *collision_programs
    )
...
@pytest.mark.parametrize(
    "old_shape,new_shape",
    [((350, 8, 64), (351, 8, 64)), ((350, 8, 64), (351, None, None))],
)
def test_random_collision_resized(
    cl_env, coord_dtype, collision_programs, old_shape, new_shape
):
    ctx, cq = cl_env
...
def test_count_err(cl_env, coord_dtype, collision_programs, size, ngroups, group_size):
    ...
    with pytest.raises(ValueError):
        e = collider.get_collisions(
            cq, coords_buf, radii_buf, n_collisions_buf, None, len(expected)
        )


@pytest.mark.parametrize(
    "dt", ["float32", np.dtype("float32"), "float64", np.dtype("float64")]
)
