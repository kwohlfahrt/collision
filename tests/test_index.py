...
def test_gather(cl_env, value_dtype, index_dtype):
    ...
    values = np.random.uniform(0, 1000, (size,) + value_dtype.shape).astype(
        value_dtype.base
    )
...
def test_scatter(cl_env, value_dtype, index_dtype):
    ...
    nindices = 30
    indexer = Indexer(ctx, value_dtype, index_dtype)
    values = np.random.uniform(0, 1000, (nindices,) + value_dtype.shape).astype(
        value_dtype.base
    )
    indices = np.random.choice(size, size=nindices, replace=False).astype(index_dtype)
    ...
    e = indexer.scatter(
        cq, nindices, values_buf, index_buf, values_out_buf, wait_for=[e]
    )
    (values_map, _) = cl.enqueue_map_buffer(
        cq,
        values_out_buf,
