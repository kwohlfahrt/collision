...
def pytest_generate_tests(metafunc):
    ...
    if "value_dtype" in params:
        metafunc.parametrize(
            "value_dtype",
            map(np.dtype, ["uint32", "float64", ("float32", 3), ("float32", 4)]),
            ids=str,
            scope="module",
        )
...
def test_sort_values(
    ...
    benchmark.pedantic(
        radix_sort,
        (cq, sorter, keys_buf, out_keys_buf, values_buf, out_values_buf),
