...
def pytest_generate_tests(metafunc):
    if "coord_dtype" in metafunc.fixturenames:
        metafunc.parametrize(
            "coord_dtype",
            list(map(dtype, [("float32", 3), ("float64", 4), "float32"])),
            scope="module",
        )
...
def test_negative_bounds(cl_env, program, coord_dtype):
    ...
    values = (
        np.random.normal(-10, 1, size=(24,) + value_dtype.shape)
...
def test_auto_program(cl_env, size, ngroups, group_size, coord_dtype):
    ...
    if coord_dtype.shape == (3,):
        ...
        value_dtype = coord_dtype
    values = np.random.normal(scale=1e8, size=(size,) + value_dtype.shape).astype(
        value_dtype.base
    )
