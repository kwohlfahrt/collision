...
kernel_args = {
    "generateBVH": [None, None, np.dtype("uint32")],
    ...
    "traverse": [None, None, np.dtype("uint32"), None, None, np.dtype("uint32")],
}
...
def pytest_generate_tests(metafunc):
    ...
    if "coord_dtype" in params:
        ...
        metafunc.parametrize("coord_dtype", map(np.dtype, ["float32"]), scope="module")
...
def test_fill_internal(cl_env, kernels):
    ...
    nodes_buf = cl.Buffer(ctx, cl.mem_flags.READ_WRITE, (2 * n - 1) * Node.itemsize)
    ids_buf = cl.Buffer(
...
def test_generate_bvh(cl_env, kernels):
    ...
    nodes_buf = cl.Buffer(ctx, cl.mem_flags.READ_WRITE, n_nodes * Node.itemsize)
    ids_buf = cl.Buffer(
        ctx, cl.mem_flags.READ_ONLY | cl.mem_flags.COPY_HOST_PTR, hostbuf=ids
    )
    ...
    np.testing.assert_equal(
        nodes_map[["data", "right_edge"]][0], expected[["data", "right_edge"]][0]
    )
...
def test_generate_odd_bvh(cl_env, kernels):
    ...
    # From Figure 3
    codes = np.array(
        [0b00001, 0b00010, 0b00100, 0b00101, 0b10011, 0b11000, 0b11001], dtype="uint32"
    )
    ids = np.arange(len(codes), dtype="uint32")
    n_nodes = len(codes) * 2 - 1
    ...
    np.testing.assert_equal(nodes_map[1:leaf], expected[1:])
    np.testing.assert_equal(
        nodes_map[["right_edge", "data"]][0], expected[["right_edge", "data"]][0]
    )
...
def test_compute_bounds(cl_env, kernels, coord_dtype):
    ...
    coords = np.array(
        [[0.0, 1.0, 3.0], [4.0, 1.0, 8.0], [-4.0, -6.0, 3.0], [-5.0, 0.0, -1.0]],
        dtype=coord_dtype,
    )
    ...
    clear_flags = cl.enqueue_fill_buffer(
        cq,
        flags_buf,
        np.zeros(1, dtype="uint32"),
        0,
        len(nodes) * np.dtype("uint32").itemsize,
    )
    calc_leaf_bounds = kernels["leafBounds"](
        cq,
        (roundUp(len(coords), 32),),
        None,
        bounds_buf,
        coords_buf,
        radii_buf,
...
def test_codes(cl_env, kernels, coord_dtype):
    ...
    coord_range = np.array([coords.min(axis=0), coords.max(axis=0)], dtype=coords.dtype)
    expected = np.array(
    ...
    coords_map[..., :3] = coords
    del coords_map
    range_buf = cl.Buffer(ctx, cl.mem_flags.READ_ONLY, 2 * 4 * coord_dtype.itemsize)
...
def test_traverse(cl_env, kernels, coord_dtype):
    ...
    clear_flags = cl.enqueue_fill_buffer(
        ...
        np.zeros(1, dtype="uint32"),
        0,
        n_nodes * np.dtype("uint32").itemsize,
    )
    ...
    clear_collisions = cl.enqueue_fill_buffer(
        cq,
        collisions_buf,
        np.array(np.iinfo(np.uint32).max, dtype="uint32"),
        0,
        n_collisions * 2 * np.dtype("uint32").itemsize,
    )
