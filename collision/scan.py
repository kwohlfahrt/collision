...
class PrefixScanner:
    ...
    def check_size(size, group_size):
        ...
        if size % (group_size * 2):
            raise ValueError(
                "Size must be multiple of 2 * group_size ({})".format(group_size)
            )
    ...
    def resize(self, size=None, group_size=None):
        ...
        self._block_sums_bufs = [
            ...
            for new_len, old_len, old_buf in zip_longest(
                self.block_lengths, old_block_lengths, self._block_sums_bufs
            )
    ...
    def prefix_sum(self, cq, values_buf, wait_for=None):
        ...
        e = self.program.kernels["local_scan"](
            ...
            (self.group_size,),
            values_buf,
            cl.LocalMemory(local_size),
            self._block_sums_bufs[0],
            wait_for=wait_for,
        )
        ...
        e = self.program.kernels["local_scan"](
            cq,
            (1,),
            (self.block_lengths[-1] // 2,),
            self._block_sums_bufs[-1],
            cl.LocalMemory(local_size),
            None,
            g_times_l=True,
            wait_for=[e],
        ...
        for size, in_buf, out_buf in zip(reversed(self.block_lengths[:-1]), *bufs):
            e = self.program.kernels["block_scan"](
                cq, (size // 2,), (self.group_size,), out_buf, in_buf, wait_for=[e]
            )
