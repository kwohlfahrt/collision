...
class CollisionProgram(SimpleProgram):
    ...
    kernel_args = {
        ...
        "traverse": [None, None, dtype("uint32"), None, None, dtype("uint32")],
    }
...
class Collider:
    ...
    def __init__(
        ...
        self.sorter = RadixSorter(
            ...
            self.padded_size,
            group_size,
            key_dtype=self.code_dtype,
            value_dtype=self.id_dtype,
    ...
    def get_collisions(
        ...
        if self.padded_size != self.size:
            fill_codes.append(
                cl.enqueue_fill_buffer(
                    cq,
                    self._codes_bufs[0],
                    array(iinfo(self.code_dtype).max, dtype=self.code_dtype),
                    0,
                    self.padded_size * self.code_dtype.itemsize,
                )
            )
        fill_ids = self.program.kernels["range"](
            cq, (self.padded_size,), None, self._ids_bufs[0]
        )
        clear_flags = cl.enqueue_fill_buffer(
            cq,
            self._flags_buf,
            zeros(1, dtype=self.flag_dtype),
            0,
            self.n_nodes * self.flag_dtype.itemsize,
        )
        clear_n_collisions = cl.enqueue_fill_buffer(
            cq,
            n_collisions_buf,
            zeros(1, dtype=self.counter_dtype),
            0,
        ...
        calc_codes = self.program.kernels["calculateCodes"](
            cq,
            (roundUp(self.size, self.group_size),),
            None,
            self._codes_bufs[0],
            coords_buf,
            self._bounds_buf,
        ...
        generate_bvh = self.program.kernels["generateBVH"](
            cq,
            (roundUp(self.size - 1, self.group_size),),
            None,
            self._codes_bufs[1],
            self._nodes_buf,
            self.size,
