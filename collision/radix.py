...
class RadixSorter:
    ...
    def __init__(
        ...
        self.scanner = PrefixScanner(
            ctx, self.histogram_len, self.group_size, scan_program
        )
    ...
    def check_size(size, group_size, radix_bits, key_dtype):
        ...
        if group_size != nextPowerOf2(group_size):
            raise ValueError(
                "Group size ({}) must be a power of two".format(group_size)
            )
        if size % (group_size * 2):
            raise ValueError(
                "Size ({}) must be multiple of 2 * group_size ({})".format(
                    size, group_size
                )
            )
        if (key_dtype.itemsize * 8) % radix_bits:
            raise ValueError(
                "Radix bits ({}) must evenly divide item-size ({})".format(
                    radix_bits, key_dtype.itemsize * 8
                )
            )
        if (2**radix_bits) > group_size * 2:
            raise ValueError(
                "2 ^ radix_bits ({}) must be less than 2 * group_size ({})".format(
                    radix_bits, group_size
                )
            )
    ...
    def sort(
        ...
        local_values = cl.LocalMemory(self.group_size * 2 * value_size)
